...
fixed3 ComputeDirLightShaftColor(float2 screenUV, fixed fogFactor, float4 lightViewportPos, fixed3 shaftColor)
{
	...
	UNITY_BRANCH
	if (WM_FOG_SUN_SHAFT_ENABLE && WM_CAMERA_RENDER_MODE_NORMAL && _WeatherMakerDirLightCount > 0 && _FogSunShaftsParam1.y > 0)// && lightViewportPos.z >= 0.0)
	{

#if defined(UNITY_COLORSPACE_GAMMA)

		static const float shaftBrightness = _FogSunShaftsParam1.z * 0.3;

#else

		static const float shaftBrightness = _FogSunShaftsParam1.z;
		...
		color *= shaftColor * _FogSunShaftsParam1.w * _FogSunShaftsTintColor.rgb * lightViewportPos.w * shaftBrightness * fogFactorReducer1 * fogFactorReducer2;
	}
