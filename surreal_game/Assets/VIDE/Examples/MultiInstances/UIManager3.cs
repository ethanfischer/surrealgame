...
using VIDE_Data; //<--- Import to use easily call VD2 class
...
{
    public string dialogueNameToLoad; //A very inappropiate way to specify the dialogue to load
    //Remember you can still call VD.GetDialogues to return a string array of dialogues.
    //Or use a VIDE_Assign component to begin the conversation
    ...
    public void ButtonAction(int dialogueIndex)
    {
        ...
        if (!data.isActive)
        {
            ...
            data.BeginDialogue(dialogueNameToLoad);
